...
def main(
    ...
    if query_option == "user":
        ...
        # Save to json
        save_json(Path("data/users") / f"{query_input}_{start_date}.json", user_data)
        ...
        if len(search_data) == 0:
            ...
            save_json(
                Path("data/search")
                / f"{query_input.replace(' ','_')}_{start_date}.json",
                search_data,
            )
        ...
        if len(comments_data) == 0:
            ...
            save_json(
                Path("data/comments") / f"{query_input}_{start_date}.json",
                comments_data,
            )
